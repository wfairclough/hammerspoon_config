...
local function createChromeMenuItem()
	...
	if chromeMenu then
		...
		chromeMenu:setMenu({
			{ title = "Copy URL", fn = copyChromeURL, shortcut = "c" },
		})
	end

	log.i("Chrome menu created")
...
local chromeAppWatcher = hs.application.watcher.new(function(name, event, app)
	log.i(name, event, app)
...
print(chromeAppWatcher)
chromeAppWatcher:start()



log.i("Chrome module loaded")
